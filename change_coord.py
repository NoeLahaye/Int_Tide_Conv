'''
CV 2017/03/22 : from http://stackoverflow.com/questions/3798333/image-information-along-a-polar-coordinate-system
'''
...
#import Image # CV : only for importing a png 
...
def reproject_image_into_polar(data, origin=None, theta_shift=0.):
    """Reprojects a 3D numpy array ("data") into a polar coordinate system.
    "origin" is a tuple of (x0, y0) and defaults to the center of the image.
    theta_shift: apply rotation to the field (radians) 
    (but theta is unshifted, otherwise there would be no rotation)"""
    ny, nx = data.shape[:2]
    if origin is None:
        origin = (nx//2, ny//2)
    ...
    theta_grid, r_grid = np.meshgrid(theta_i-theta_shift, r_i)
    ...
    bands = []
    for band in data.T:
        print data.T.shape,band.shape,coords.shape
        zi = sp.ndimage.map_coordinates(band, coords, order=1)
        #zi = sp.ndimage.interpolation.map_coordinates(band, coords, order=1) # CV : found more this one 
        bands.append(zi.reshape((nx, ny)))
    output = np.dstack(bands)
    '''
    # CV : ok to reproject in on step for 'small' matrices (tested until 800x800)  
    output = sp.ndimage.interpolation.map_coordinates(data, coords, order=1)
